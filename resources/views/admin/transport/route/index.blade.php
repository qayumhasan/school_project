                                ...
                                <tr class="text-center">
                                    ...
                                    <td>
                                        @if (json_decode($userPermits->transport_module, true)['route']['edit'] == 1)
                                            @if($route->status==1)
                                                {{ $route->status }}<a href="{{ route('admin.route.status.update', $route->id ) }}"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="add_route_form" action="{{ route('admin.route.store') }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label><b>Name :</b> </label>
                        <input type="text" class="form-control" placeholder="Route name" name="name" id="name">
                        <span class="error error_name"></span>
                    </div>

                    <div class="form-group">
                        <label><b>Fare :</b> </label>
                        <input type="number" class="form-control" id="fare" placeholder="Fare Amount" name="fare">
                        <span class="error error_fare"></span>
                    </div>
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content edit_content">
            ...
            <div class="modal-body">
                <form id="edit_route_form" action="{{ route('admin.route.update') }}" method="POST"
                    ...
                    <div class="form-group">
                        <label><b>Fare :</b></label>
                        <input type="number" class="form-control fare" name="fare" id="e_fare">
                        <span class="error e_error_fare"></span>
                    </div>
                    ...
                    <div class="form-group text-right">
                        ...
                        <button type="submit" class="btn btn-sm loading_button btn-blue">Loading...</button>
                        @if (json_decode($userPermits->transport_module, true)['route']['edit'] == 1)
                            <button type="submit" class="btn btn-sm submit_button btn-sm btn-blue">Submit</button>
                        @endif 
                    </div>
                </form>
            </div>
...
@push('js')
    ...
    <script>
		$(document).ready(function () {
			...
			$(document).on('submit', '#add_route_form', function(e){
				e.preventDefault();
    ...
    <script>
		$(document).ready(function () {
			...
			$(document).on('submit', '#edit_route_form', function(e){
				e.preventDefault();
				var url = $(this).attr('action');
                var type = $(this).attr('method');
                var data = $(this).serialize();
