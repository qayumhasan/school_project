        ...
        <div class="row">
                    <div class="col-md-6">
                        <div class="panel_title">
                            <span class="panel_icon"><i class="fas fa-border-all"></i></span><span>News List</span>
                        </div>
                    </div>
                    <div class="col-md-6 text-right">
                        <div class="panel_title">
                            <a href="#" class="btn btn-sm btn-success" data-toggle="modal" data-target="#myModal1"><i
                                    class="fas fa-plus"></i></span> <span>Add News</span></a>
                        </div>
                    </div>
                </div>
        </div>
        <form action="{{route('admin.front.news.multidelete')}}" method="post">
            @csrf
                ...
                <table id="dataTableExample1" class="table table-bordered table-striped table-hover mb-2">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($row->status == 1)
                                <a href="{{ route('admin.news.status', $row->id) }}" class="btn btn-success btn-sm ">
                                    <i class="fas fa-thumbs-up"></i></a>
                                @else
                                <a href="{{ route('admin.news.status', $row->id ) }}" class="btn btn-danger btn-sm">
                            ...
                            <td>
                                ...
                                <a id="delete" href="{{route('admin.news.delete',$row->id)}}" class="btn btn-danger btn-sm text-white" title="Delete">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-horizontal" action="{{ route('admin.front.news.store') }}" method="POST">
                    @csrf
                    <div class="form-group row">
                        <label for="inputEmail3" class="col-sm-3 col-form-label text-right">Title:</label>
                        <div class="col-sm-8">
                            <input required type="text" class="form-control" placeholder="news Title" name="title" required>
                        </div>
                    </div>
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-horizontal" action="{{ route('admin.front.news.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                     <div class="form-group row">
                        <label for="inputEmail3" class="col-sm-3 col-form-label text-right">Title:</label>
                        <div class="col-sm-8">
                            ...
                            <input required type="text" class="form-control" id="title" placeholder="news Title" name="title" required>
                        </div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.edit_route').on('click', function() {
            ...
            if (id) {
                $.ajax({
                    url: "{{ url('/admin/front/cms/news/edit/') }}/" + id,
                    type: "GET",
                    dataType: "json",
