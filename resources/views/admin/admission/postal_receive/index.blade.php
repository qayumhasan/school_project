            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="panel_title">
                        <span class="panel_icon"><i class="fas fa-border-all"></i></span><span>Postal Recevie</span>
                    </div>
                </div>
                <div class="col-md-6 text-right">
                    <div class="panel_title">
                        <a href="#" class="btn btn-sm btn-success" data-toggle="modal" data-target="#myModal1"><i class="fas fa-plus"></i></span> <span>Add Postal Recevie</span></a>
                    </div>
                </div>
            </div>
        </div>
        <form action="{{route('admin.postal.receives.multi.delete')}}" method="post">
            @csrf
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if($row->status == 1)
                                    <a href="{{ route('admin.postal.receives.status', $row->id) }}" class="btn btn-success btn-sm ">
                                        <i class="fas fa-thumbs-up"></i></a>

                                    @else
                                    <a href="{{ route('admin.postal.receives.status', $row->id ) }}" class="btn btn-danger btn-sm">
                                ...
                                <td>
                                <a class="view_route btn btn-sm btn-blue text-white" data-action="{{route('admin.postal.receives.view',$row->id)}}" data-id="{{$row->id}}" title="edit" data-toggle="modal" data-target="#viewmodal"><i class="fa fa-eye" aria-hidden="true"></i></a> 
                                    | <a class="edit_route btn btn-sm btn-blue text-white" data-action="{{route('admin.postal.receives.edit',$row->id)}}" data-id="{{$row->id}}" title="edit" data-toggle="modal" data-target="#editModal"><i class="fas fa-pencil-alt"></i></a> |
                                    <a id="delete" href="{{route('admin.postal.receives.delete',$row->id)}}" class="btn btn-danger btn-sm text-white" title="Delete">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h4 class="modal-title">Add Postal Recevie</h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>

            <!-- Modal body -->
            <div class="modal-body">
                <form class="form-horizontal" action="{{ route('admin.postal.receives.store') }}" enctype="multipart/form-data" method="POST">
                    @csrf
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Update Postal Recevie</h5>
